...
### 바꾼 점
기존에는 tomato 안에 0이 있는지, max_day 또한 반복문으로 구현하여 불필요한 시간이 소요되었다. bfs() 안에 변수를 선언하여 추후에 반복문을 쓰지 않도록 개선하여 pypy3로 재제출함.
